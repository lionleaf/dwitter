...
def v0_DweetView(request,dweetId):
    ...
    try:
        ...
        json = {
            ...
            "remix_of" : dweet.reply_to.id if dweet.reply_to!=None else None,
        }
    except:
        json = {
            "detail" : "Not found.",
...
def v0_DweetsView(request):
    ...
    json = {
            "_" : page,
                        "_2" : lastpage,
        "count" : len(results),
        "next": None if (page>lastpage) else "https://www.dwitter.net/api/dweets/?limit="+str(limit)+"&offset="+str(offset+limit),
        "previous": None if (page==0) else "https://www.dwitter.net/api/dweets/?limit="+str(limit)+("&offset="+str(offset-limit) if offset-limit>0 else ""),
        "results" : results,
    }
